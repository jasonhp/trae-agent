...
"""Novita client wrapper with tool integrations"""
...
class NovitaClient(BaseLLMClient):
    """Novita client wrapper with tool schema generation."""

    def __init__(self, model_parameters: ModelParameters):
        super().__init__(model_parameters)

        if self.api_key == "":
            self.api_key: str = os.getenv("NOVITA_API_KEY", "")

        if self.api_key == "":
            raise ValueError(
                "Novita API key not provided. Set NOVITA_API_KEY in environment variables or config file."
            )
    ...
    def chat(
        ...
        """Send chat messages to Novita with optional tool support."""
        novita_messages = self.parse_messages(messages)
        if reuse_history:
            self.message_history = self.message_history + novita_messages
        else:
            self.message_history = novita_messages
        ...
        if response is None:
            raise ValueError(
                f"Failed to get response from Novita after max retries: {error_message}"
            )
    ...
    def parse_messages(
        ...
        novita_messages: list[ChatCompletionMessageParam] = []
        for msg in messages:
            if msg.tool_call:
                novita_messages.append(
                    ChatCompletionFunctionMessageParam(
                        content=json.dumps(
                            {
